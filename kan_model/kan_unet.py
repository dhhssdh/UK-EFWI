...
class KANU_Net(nn.Module):
    ...
    def forward_once(self, x):
        ...
        logits = self.outc(x)
        return logits
    def forward(self, xx,yy,zz):
        ou1 = self.forward_once(xx.unsqueeze(0).unsqueeze(0))
        #vp = self.mp_min + torch.abs(ou1) * (self.mp_max - self.mp_min)
        ou2 = self.forward_once(yy.unsqueeze(0).unsqueeze(0))
        #vs = self.ms_min + torch.abs(ou2) * (self.ms_max - self.ms_min)
        ou3 = self.forward_once(zz.unsqueeze(0).unsqueeze(0))
        #rho = self.mp_min + torch.abs(ou2) * (self.mrho_max - self.mrho_min)
        return ou1.squeeze(),ou2.squeeze(),ou3.squeeze()
if __name__ == "__main__":
     device = 'cuda' if torch.cuda.is_available() else 'mps' if torch.backends.mps.is_available() else 'cpu'
     # print(device)
     model = KANU_Net(1, 1, 'mps').to(device)
     #print(model)
     x = torch.randn((1, 1, 136, 401)).to(device)
     print(model(x,x,x)[1].shape)
